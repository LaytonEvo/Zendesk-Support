...
def test_a_reply_sent_as_written_counts_as_used(store):
    ...
    a = metrics.draft_adoption(store)
...
def test_old_tickets_fall_outside_the_window(store):
    ...
    assert metrics.usage(store, days=30)["tickets"] == 0
...
def test_the_page_renders_with_no_data_at_all(store):
    ...
    html = dashboard.render(metrics.report(store, 30), 30)
...
def test_the_dashboard_opens_with_its_own_token(api):
    r = api.get("/dashboard", params={"token": "dash-tok"})
    assert r.status_code == 200 and "Support dashboard" in r.text


def test_a_wrong_token_is_refused(api):
    assert api.get("/dashboard", params={"token": "nope"}).status_code == 401
    assert api.get("/dashboard").status_code == 401


def test_the_dashboard_token_does_not_unlock_anything_else(api):
    """It gets shared with whoever wants to look. It must not carry admin."""
